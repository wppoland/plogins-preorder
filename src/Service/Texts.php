 ...
 * site.
 *
 * The pre-order button label used to be an English sentence in
 * config/defaults.php. A string in a config array is never wrapped in a gettext
 * call, so it is not in the .pot and no translator can reach it, and because the
 * defaults are merged under the stored option the key was always present and
 * never empty. The `__()` fallback that sat next to it in Settings looked like
 * the problem was handled, but it could never fire.
 *
 * The packaged default is now empty, meaning "use the string below". A merchant
 * who types their own label still wins, and what they typed is stored as typed.
 */
final class Texts
{
    /**
